...
def auc(x, y):
    ...
    if x.shape[0] < 2:
        ...
        area = direction * (np.trapezoid(y, x) if hasattr(np, 'trapezoid') else np.trapz(y, x))
...
def bvaluation(reference: Path, predictions: list, outpath=".", dataset=True, target=False, bootstrap=False,
    ...
    for m in tqdm(metrics_to_write, desc="Writing metrics"):
        if dataset is True:
            if m not in dts_data:
                logging.warning("Skipping metric '{}': no predictor produced a valid threshold for it.".format(m))
                continue
