...
def create_model(config: dict, device: str):
    ...
    if training_phase == 1:
        # Phase 1: Create disorder-only model (with recycling)
        num_recycles = config['model'].get('num_recycles', 0)
        if num_recycles > 0:
            logger.info(f"Creating Phase 1 Recycle model (num_recycles={num_recycles}, total passes=1+{num_recycles})")
            ...
            logger.info("Creating Phase 1 model (backbone + disorder head, no recycling)")
            model = cascDP_Phase1(
                backbone=backbone,
                device=device,
                context_type=config['model'].get('context_type', 'bigru'),
        ...
        # Determine Phase 1 context type (critical for loading weights correctly)
...
def main():
    parser = argparse.ArgumentParser(description='Train cascDP model with iterative recycling (Phase 1)')
    ...
    recycle_proj_lr = optimizer_config.get('recycle_proj_lr', base_lr)
    weight_decay = optimizer_config.get('weight_decay', 0.01)

    # Separate param group for recycle_proj (starts from zero, benefits from higher LR)
    recycle_proj_params = [p for n, p in model.named_parameters() if p.requires_grad and 'recycle' in n]
    base_params        = [p for n, p in model.named_parameters() if p.requires_grad and 'recycle' not in n]
    param_groups = [
        {'params': base_params,        'lr': base_lr},
        {'params': recycle_proj_params, 'lr': recycle_proj_lr},
    ]
    logger.info(f"Optimizer param groups: base_lr={base_lr} ({len(base_params)} tensors), "
                f"recycle_proj_lr={recycle_proj_lr} ({len(recycle_proj_params)} tensors)")
